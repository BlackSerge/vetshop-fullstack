"""
pedidos/selectors.py

Capa de selectores (queries optimizadas) para el módulo de pedidos.
...
def get_order_by_id(order_id: int) -> Optional[Order]:
    """Obtiene una orden por ID con items prefetched."""
    try:
...
def get_user_orders(user_id: int, status: str = None) -> QuerySet:
    """Obtiene todas las órdenes de un usuario (filtrable por status)."""
...
def get_all_orders(status: str = None) -> QuerySet:
    """Obtiene todas las órdenes (admin - filtrable por status)."""
...
def order_exists_for_payment_intent(payment_intent_id: str) -> bool:
    """Verifica si una orden existe para un payment_intent (prevenir duplicados)."""
...
def get_order_by_payment_intent(payment_intent_id: str) -> Optional[Order]:
    """Obtiene una orden por su payment_intent Stripe."""
    try:
...
def get_total_user_spending(user_id: int) -> float:
    """Calcula el gasto total histórico de un usuario (solo órdenes pagadas)."""
...
def get_total_orders_count() -> int:
    """Obtiene el total de órdenes (sin importar status)."""
...
def get_sales_last_n_days(days: int) -> float:
    """Calcula ventas de los últimos N días."""
...
def get_orders_last_n_days(days: int) -> int:
    """Obtiene cantidad de órdenes de los últimos N días."""
    date_threshold = timezone.now() - timedelta(days=days)
...
def get_order_summary_by_status(days: Optional[int] = None) -> QuerySet:
    """Obtiene resumen de órdenes por estado (filtrable por periodo)."""
...
def get_period_metrics(days: int) -> dict:
    """
    Obtiene métricas para un periodo (ventas, órdenes, clientes, items).
    """
    threshold = timezone.now() - timedelta(days=days)
    qs = Order.objects.filter(created_at__gte=threshold)
    paid_qs = qs.filter(status="PAID")
    
    # Items vendidos (unidades totales)
...
def get_previous_period_metrics(days: int) -> dict:
    """
    Obtiene métricas para el periodo ANTERIOR al actual.
    Ej: Si días=30, obtiene de hace 60 a hace 30 días.
    """
    now = timezone.now()
...
def get_dashboard_trends(days: int) -> dict:
    """
    Calcula las tendencias (percentages) comparando periodo actual vs anterior.
    """
...
def get_dashboard_analytics(days: int = 30) -> dict:
    """
    Obtiene métricas completas para un período específico.
    """
    date_threshold = timezone.now() - timedelta(days=days)
...
def get_dynamic_chart_data(days: int = 7) -> List[dict]:
    """
    Obtiene datos de ventas agrupados dinámicamente según el rango de días.
    - <= 30 días: Por día
    - <= 90 días: Por semana
    - > 90 días: Por mes
    """
...
def get_sales_by_category_period(days: int = 30) -> List[dict]:
    """Obtiene ventas agregadas por categoría en un periodo."""
    date_threshold = timezone.now() - timedelta(days=days)
    return list(
        OrderItem.objects.filter(order__status="PAID", order__created_at__gte=date_threshold)
...
def get_order_items(order_id: int) -> QuerySet:
    """Obtiene todos los ítems de una orden."""
...
def get_order_item_count(order_id: int) -> int:
    """Obtiene la cantidad total de ítems en una orden."""
