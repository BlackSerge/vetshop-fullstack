...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Usamos productos_todos.json si existe, sino procesamos todos los *.json
        json_files = list(data_dir.glob('*.json'))
        if not json_files:
            self.stdout.write(self.style.ERROR('No se encontraron archivos JSON en data/products_db'))
            return

        # Si existe productos_todos.json, priorizamos ese para evitar duplicidad si otros son parciales
    ...
    def process_product(self, item):
        try:
            ...
            # 3. Manejar Imágenes (opcional, solo si no existen)
            if item.get('imagenes') and created:
                for img_data in item.get('imagenes'):
                    # No descargamos la imagen aquí por simplicidad y porque suelen ser placeholders
                    # Solo guardamos la referencia si es necesario, pero ImagenProducto usa CloudinaryField
                    # Para esta migración local, podríamos simplemente saltar la descarga real de archivos
                    pass
