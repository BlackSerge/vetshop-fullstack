...
export default function ProductsPage() {
  ...
  const cartCount = useCartStore((state: any) => state.count);
  ...
  const searchQueryTimeout = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const handleSearchInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    searchQueryTimeout.current = setTimeout(async () => {
      ...
      if (query.length >= 1) {
        try {
            const data = await productService.getProducts(`search=${query}&page_size=5`);
            const matches = data.results.map((p: any) => p.nombre);
