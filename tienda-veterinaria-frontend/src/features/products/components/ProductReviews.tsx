...
export default function ProductReviews({ productId, reviews = [], onReviewAdded }: { productId: number; reviews?: any[]; onReviewAdded: () => void }) {
  const { isAuthenticated } = useAuthStore() as any;
  const theme = useThemeStore((state: any) => state.theme);
  const isDark = theme === 'dark';

  const { rating, setRating, comment, setComment, submitting, submitReview } = useProductReview(productId, onReviewAdded);
