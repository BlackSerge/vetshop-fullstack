...
export default function AdminDashboardPage(): JSX.Element {
  ...
  const fetchStats = useCallback(async (selectedPeriod: PeriodType): Promise<void> => {
        ...
        <div className={`lg:col-span-2 p-6 rounded-2xl border ${cardBg} transition-all hover:shadow-lg`}>
            ...
            <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={(stats.chart_data as unknown as any[]) || []}>
                        <defs>
        ...
        <div className={`p-6 rounded-2xl border ${cardBg} transition-all hover:shadow-lg flex flex-col`}>
            ...
            <div className="space-y-5 flex-grow">
                {(stats.top_products || []).map((prod: any, i: number) => {
                    const productData = prod as ProductData;
                    return (
                    <div key={i} className="group flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold truncate">{productData.product__nombre}</p>
                            <p className="text-[10px] text-gray-500">{productData.total_quantity} unidades vendidas</p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-black text-purple-600">{formatPrice(productData.total_revenue || 0)}</p>
                        </div>
                    </div>
                    );
                })}
...
function MetricCard({ title, value, icon, trend, color, isDark, isLoading }: MetricCardProps): JSX.Element {
    const colorClasses = {
