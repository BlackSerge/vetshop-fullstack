...
import { Edit, Trash2, Plus, Image as ImageIcon, Search, Settings } from 'lucide-react';
import { toast } from 'react-toastify';
import { useThemeStore } from '@/shared';
import adminService from '../services/adminService';
import AdminTable from '../components/AdminTable';
import { ConfirmModal } from '@/shared';
import type { ProductoCreateUpdate, SelectOption, Categoria } from '../types';

interface ProductWithDisplay extends ProductoCreateUpdate {
  categoria_display_name?: string;
...
export default function AdminProductListPage(): JSX.Element {
  ...
  const [productToDelete, setProductToDelete] = useState<ProductWithDisplay | null>(null);
  ...
  const fetchProducts = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      let productsArray: ProductWithDisplay[] = [];
      let totalCount = 0;
  ...
  useEffect(() => {
    const fetchCategoriesOptions = async (): Promise<void> => {
      try {
        const response = await adminService.getCategories();
        const categoriesData = response.results || [];
        const options: SelectOption[] = [
          { value: '', label: 'Todas las categorías' },
          ...categoriesData.map((cat: Categoria) => ({ value: cat.slug || '', label: cat.nombre }))
        ];
  ...
  const handleDeleteClick = (product: ProductWithDisplay): void => {
  ...
  const renderProductActions = (row: Record<string, any>): JSX.Element => {
    const product = row as ProductWithDisplay;
    ...
    <div className="w-full pb-32 animate-fadeIn">
      ...
      <div className={`p-2 rounded-3xl mb-8 border flex flex-col md:flex-row gap-3 shadow-sm ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
          ...
          <div className={`relative md:w-72 flex items-center gap-2 p-2 rounded-2xl border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-50 shadow-inner'}`}>
              <Settings className="text-gray-400 ml-2" size={18} />
              <select
      ...
      <AdminTable
        ...
        data={products.map(prod => {
          const precioOferta = prod.precio_oferta ? parseFloat(String(prod.precio_oferta)) : null;
          ...
          return {
            ...
            categoria_display_name: (
                <span className={`px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wide border ${isDark ? 'bg-gray-800 border-gray-600 text-gray-300' : 'bg-gray-100 border-gray-200 text-gray-600'}`}>
                    {(prod as any).categoria_info?.nombre || '—'}
                </span>
            ),
            precio_formateado: (
              <span className="font-mono font-bold text-purple-500">
                ${(precioOferta || precio).toFixed(2)}
              </span>
            ),
            stock: (
                <span className={`font-bold ${stock < 5 ? 'text-red-500' : isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    {stock}
                </span>
            ),
