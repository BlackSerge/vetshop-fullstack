...
export default function AdminPanelPage() {
  ...
  // Auto-rotate category highlights every 3 seconds
  useEffect(() => {
    if (stats?.sales_by_category?.length > 1) {
      const interval = setInterval(() => {
        setActiveIndex((current) => (current + 1) % stats.sales_by_category.length);
      }, 3000);
  ...
  // Calculate total orders in period to avoid showing empty status summary
  const totalOrdersInPeriod = stats.orders_by_status?.reduce((acc, curr) => acc + curr.count, 0) || 0;
      ...
      <section className="w-full space-y-8 animate-fadeIn pb-24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <StatCard 
                ...
                icon={DollarSign} 
                trend={stats.trends?.sales_trend}
            ...
            <StatCard 
                ...
                icon={ShoppingBag} 
                trend={stats.trends?.items_trend}
            ...
            <StatCard 
                ...
                icon={Users} 
                trend={stats.trends?.customers_trend}
                subTitle="Activos"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className={`lg:col-span-2 p-6 rounded-3xl border shadow-lg ${cardStyle}`}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    ...
                    <div className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-lg border ${stats.trends?.sales_trend >= 0 ? (isDark ? 'bg-green-900/20 text-green-300 border-green-800' : 'bg-green-50 text-green-600 border-green-100') : (isDark ? 'bg-red-900/20 text-red-300 border-red-800' : 'bg-red-50 text-red-600 border-red-100')}`}>
                        <TrendingUp size={14} className={stats.trends?.sales_trend < 0 ? 'rotate-180' : ''} /> {stats.trends?.sales_trend}% Crecimiento
                    </div>
                </div>
                            ...
                            <div key={index} className={`flex items-center gap-4 group p-3 rounded-2xl transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-bold truncate ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
                                        {prod.product__nombre}
                                    </p>
                                    <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                                        {prod.total_quantity} ventas
                                    </p>
                                </div>
        ...
        {/* 3. Sales by Category Bar Chart - Auto Rotating */}
        <div className={`p-6 rounded-3xl border shadow-lg ${cardStyle}`}>
            ...
            {/* Resumen de Estados (Mini List) - Only visible if there are orders in THIS period */}
            {totalOrdersInPeriod > 0 && stats.orders_by_status?.length > 0 && (
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center px-2 animate-in fade-in slide-in-from-bottom-2 duration-1000">
                    {stats.orders_by_status.map((status, i) => (
